...
fn plus_one_compilable_with_underbar(value: Option<i32>) -> Option<i32> {
    match value {
        Some(i) => Some(i + 1),
        _ => None,
    }
...
fn plus_one_compilable_with_other(value: Option<i32>) -> Option<i32> {
    match value {
        Some(i) => Some(i + 1),
        other => None,
    }
...
fn run_temperature_conversion() {
    ...
    io::stdin()
        .read_line(&mut input)
        .expect("Failed in read_line");

    let celsius: f32 = input
        .trim()
        .parse()
        .expect("Fail to parse. Your input is not in format of number");
    println!(
        "Converted value of celsius to farenheit is : {}",
        ((celsius * 1.8) + 32.0)
    );
}
